/**
 * @file ssd1306_i2c_driver.cpp
 * @author rohirto
 * @brief I2C Screen Driver
 * @version 0.1
 * @date 2024-12-01
 * 
...
void SSD1306_send_buf(uint8_t buf[], int buflen) {
    ...
    uint8_t *temp_buf = (uint8_t*)pvPortMalloc(buflen + 1);
...
static void SetPixel(uint8_t *buf, int x,int y, bool on) {
    configASSERT(x >= 0 && x < SSD1306_WIDTH && y >=0 && y < SSD1306_HEIGHT);
...
#warning i2c / SSD1306_i2d example requires a board with I2C pins
    ...
    frame_area.buflen = (frame_area.end_col - frame_area.start_col + 1)*(frame_area.end_page - frame_area.start_page + 1);
...
void SSD1306::render() 
{
    ...
    uint8_t cmds[] = {
        ...
        frame_area.start_col,
        frame_area.end_col,
        SSD1306_SET_PAGE_ADDR,
        frame_area.start_page,
        frame_area.end_page
    };

    SSD1306_send_cmd_list(cmds, count_of(cmds));
    SSD1306_send_buf(buf, frame_area.buflen);
}

void SSD1306::set_all_pixels()
{
    SSD1306_send_cmd(SSD1306_SET_ALL_ON);    // Set all pixels on
}

void SSD1306::go_back_to_previous_state()
{
    SSD1306_send_cmd(SSD1306_SET_ENTIRE_ON); // go back to following RAM for pixel state-
}
void SSD1306::clear_all_pixels()
{
    // SSD1306_send_cmd(SSD1306_SET_ENTIRE_ON); // go back to following RAM for pixel state-
    memset(buf, 0, SSD1306_BUF_LEN);
    render();
...
void SSD1306::render_image(int image_height, int image_width, uint8_t* imag_buf)
{
    frame_area.start_page = 0;
    frame_area.end_page = (image_height / SSD1306_PAGE_HEIGHT)  - 1;
    frame_area.start_col = 0;
    frame_area.end_col = image_width - 1;

    frame_area.buflen = (frame_area.end_col - frame_area.start_col + 1) * (frame_area.end_page - frame_area.start_page + 1);

    uint8_t offset = 5 + image_width; // 5px padding
    memcpy(buf, imag_buf, frame_area.buflen);
    render();

}

void SSD1306::SSD1306_scroll(bool on) 
{
...
void SSD1306::write_string(const std::string& str)
{
    ...
    while ((y <= SSD1306_HEIGHT - 8) && (str_size >= 0))
    {
        if (x > SSD1306_WIDTH - 8)
        {
            y+=8;
        }
        WriteChar(buf, x, y, *c_str++);
        x+=8;
        str_size --;
...
void SSD1306::write_string_array(const std::vector<std::string>& text)
{
    clear_all_pixels();
    ...
    {
        WriteString(buf, x, y, line.c_str());
        y+=8;
    }
    x = 5;
    render();
